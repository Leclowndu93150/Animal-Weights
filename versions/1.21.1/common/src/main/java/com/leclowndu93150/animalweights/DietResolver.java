...
public final class DietResolver {
    ...
    public static final TagKey<EntityType<?>> LIVESTOCK = TagKey.create(Registries.ENTITY_TYPE, ResourceLocation.fromNamespaceAndPath("animalweights", "livestock"));
    ...
    private static TagKey<EntityType<?>> tag(String name) {
        return TagKey.create(Registries.ENTITY_TYPE, ResourceLocation.fromNamespaceAndPath("animalweights", "diet/" + name));
    }

    public static Diet resolveByTag(EntityType<?> type) {
        if (type.is(NETHER)) return Diet.NETHER;
        if (type.is(AQUATIC)) return Diet.AQUATIC;
        if (type.is(CARNIVORE)) return Diet.CARNIVORE;
        if (type.is(HERBIVORE)) return Diet.HERBIVORE;
        if (type.is(OMNIVORE)) return Diet.OMNIVORE;
        return null;
    }
    ...
    public static Diet resolveBySpawnBiomes(Entity entity) {
        ...
        for (Holder.Reference<Biome> ref : biomes.holders().toList()) {
            ...
            for (MobCategory cat : MobCategory.values()) {
                for (MobSpawnSettings.SpawnerData entry : settings.getMobs(cat).unwrap()) {
